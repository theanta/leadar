...
export default function Jobs() {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <div className="flex shrink-0 items-center justify-between border-b border-border px-6 py-5 lg:px-10">
        <div>
              ...
              {data?.items.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-4 py-16 text-center text-muted-foreground">
                    No jobs found.{" "}
                    <button onClick={() => navigate("/search")} className="font-medium text-foreground hover:underline">
                      Start a search →
                    </button>
                  </td>
                </tr>
                ...
                data?.items.map((job) => (
                  <tr key={job.id} className="border-b border-border transition-colors hover:bg-surface-soft/60">
                    <td className="px-4 py-3">
                      <p className="max-w-xs truncate font-medium text-foreground">{job.keyword}</p>
                      {job.error_message && (
                        <p className="mt-0.5 max-w-xs truncate text-xs text-destructive">{job.error_message}</p>
                      )}
                    </td>
      ...
      <div className="flex shrink-0 items-center justify-between border-t border-border bg-surface-soft px-6 py-3 lg:px-10">
